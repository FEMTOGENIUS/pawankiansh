...
/* ═══════════════ VENUE 3D TILT ═══════════════════════════ */
(() => {
  const stage = $("#venue-3d-stage"), img = $("#venue-3d-img");
  if (!stage || !img || REDUCED) return;
  let ticking = false;
  const update = () => {
    ticking = false;
    ...
    const p = clamp(center / innerHeight, -1, 1);
    img.style.transform = `rotateX(${(6 - p * 5).toFixed(2)}deg) rotateY(${(-8 + p * 10).toFixed(2)}deg) scale(1.02)`;
  };
...
(() => {
  ...
  const buildRsvp = () => {
    ...
    slot.querySelector("#rsvp-form").addEventListener("submit", (e) => {
      ...
      const waMsg =
        `RSVP · Pawan & Anshita's Wedding\n` +
        `Name: ${data.name}\n` +
        `Attending: ${data.attending === "yes" ? "Joyfully accepts" : "Regretfully declines"}\n` +
        `Guests: ${data.count}` +
        (data.note && data.note.trim() ? `\nNote: ${data.note.trim()}` : "");
      const waUrl = "https://wa.me/918896198444?text=" + encodeURIComponent(waMsg);
...
(() => {
  ...
  /* Ornate corner flourishes — restored from the original site design.
     Injected once per box rather than hand-written four times in markup. */
  const CORNER_SVG = `<svg viewBox="0 0 100 100" fill="none">
    <path d="M4,4 C4,46 46,4 96,4" stroke="currentColor" stroke-width="1.3"/>
    <path d="M4,4 C46,4 4,46 4,96" stroke="currentColor" stroke-width="1.3"/>
    <path d="M4,4 C4,30 30,4 60,4" stroke="currentColor" stroke-width="1"/>
    <path d="M4,4 C30,4 4,30 4,60" stroke="currentColor" stroke-width="1"/>
    <circle cx="14" cy="14" r="3" fill="currentColor"/>
    <path d="M22,10 C28,6 34,10 30,16 C26,20 20,16 22,10Z" fill="currentColor" opacity=".7"/>
  </svg>`;
  document.querySelectorAll(".ornate-frame").forEach((box) => {
    const inset = document.createElement("span");
    inset.className = "frame-inset";
    box.appendChild(inset);
    ["tl", "tr", "bl", "br"].forEach((pos) => {
      const span = document.createElement("span");
      span.className = "fcorner " + pos;
